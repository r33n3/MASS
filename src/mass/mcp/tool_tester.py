...
# Delegated to the shared corpus — these module-level lists are kept
# for backward compatibility with existing callers.
from mass.sandbox.corpus.payloads import TOOL_PAYLOADS as _CORPUS

COMMAND_INJECTION_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("command_injection", [])]
PATH_TRAVERSAL_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("path_traversal", [])]
SSRF_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("ssrf", [])]
SQL_INJECTION_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("sql_injection", [])]
PROMPT_INJECTION_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("prompt_injection", [])]
TEMPLATE_INJECTION_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("template_injection", [])]
BOUNDARY_PAYLOADS: list[tuple[Any, str]] = [(p.value, p.description) for p in _CORPUS.get("boundary", [])]
EXFILTRATION_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("exfiltration", [])]
PRIVILEGE_ESCALATION_PAYLOADS: list[tuple[Any, str]] = [(p.value, p.description) for p in _CORPUS.get("privilege_escalation", [])]
INFORMATION_DISCLOSURE_PAYLOADS = [(p.value, p.description) for p in _CORPUS.get("information_disclosure", [])]
